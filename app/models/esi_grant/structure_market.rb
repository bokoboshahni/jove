...
class ESIGrant < ApplicationRecord
  class StructureMarket < ESIGrant
    self.requested_scopes = %w[esi-markets.structure_markets.v1]

    validates :grantable, presence: true
    validates :grantable_type, inclusion: { in: %w[Structure] }
  end
end
